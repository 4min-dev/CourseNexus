...
export function TopCourses({ categoryId, platform, limit = 5, title = "🔥 Популярные курсы", onDataLoaded }: TopCoursesProps) {
  const isPerfLow = useMemo(() => {
    try {
  ...
  const { data: topCourses, isLoading } = useQuery<TopCourse[]>({
    ...
    queryFn: async () => {
      ...
      const url = `/api/categories/${categoryId}/top-courses?${params.toString()}`
  ...
  const courseData = useMemo(() => {
    ...
    return topCourses.map((course, index) => {
      ...
      const repeatCount = isPerfLow ? 2 : 7;
      const tripleItems = Array.from({ length: repeatCount }, () => selectedSubcategories).flat()
  ...
  }, [topCourses, subcategories, isPerfLow])

  useEffect(() => {
    if (onDataLoaded) {
      onDataLoaded(!!topCourses && topCourses.length > 0)
    }
  }, [topCourses, onDataLoaded])
  ...
  return (
    <div className="space-y-4" data-testid="top-courses-section">
      ...
      <div className="grid gap-3 grid-cols-[100%]">
        {courseData.map(({ course, index, tripleItems }) => (
          <Link
            key={course.id}
          ...
          >
            <Card className="group relative overflow-hidden hover-elevate active-elevate-2 cursor-pointer transition-all duration-200">
              ...
              <div className="flex gap-4 p-4 pb-0 flex-wrap">
                ...
                <div className="flex-1 min-w-0 space-y-2 flex-wrap">
                  <h3 className="font-semibold text-base group-hover:text-purple-500 transition-colors">
                    {course.title}
                  </h3>

                  <div className="flex items-center gap-2 text-xs text-muted-foreground">
                    {course.authorName && <span className="line-clamp-1">{course.authorName}</span>}
              ...
              <TagsMarquee
                ...
                repeatCount={1}
                className="h-7 my-2"
                itemClassName="bg-background/60 backdrop-blur-sm border-border/50 text-foreground/90"
              />
            </Card>
          </Link>
        ))}
